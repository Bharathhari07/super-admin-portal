import { Pencil, Power, PowerOff } from 'lucide-react'
import type { Branch } from '../../types/branch'
import Badge from '../common/Badge'
import Button from '../common/Button'
import Spinner from '../common/Spinner'
import EmptyState from '../common/EmptyState'

interface BranchTableProps {
  branches: Branch[]
  isLoading: boolean
  isError: boolean
  onEdit: (branch: Branch) => void
  onToggleStatus: (branch: Branch) => void
  togglingBranchId: string | null
}

const columns = ['Branch', 'Code', 'Business Unit', 'Manager', 'Location', 'Status', 'Actions']

export default function BranchTable({
  branches,
  isLoading,
  isError,
  onEdit,
  onToggleStatus,
  togglingBranchId,
}: BranchTableProps) {
  return (
    <div className="overflow-x-auto sap-scroll">
      <table className="w-full min-w-[760px] border-collapse">
        <thead>
          <tr className="border-b border-sap-border bg-sap-bg/60 text-left">
            {columns.map((col) => (
              <th key={col} className="whitespace-nowrap px-4 py-3 text-xs font-semibold uppercase tracking-wide text-sap-text-muted">
                {col}
              </th>
            ))}
          </tr>
        </thead>
        <tbody>
          {isLoading && (
            <tr>
              <td colSpan={columns.length} className="py-10">
                <div className="flex justify-center">
                  <Spinner />
                </div>
              </td>
            </tr>
          )}
          {isError && !isLoading && (
            <tr>
              <td colSpan={columns.length} className="py-10 text-center text-sm text-sap-danger">
                Couldn't load branches. Please try again.
              </td>
            </tr>
          )}
          {!isLoading && !isError && branches.length === 0 && (
            <tr>
              <td colSpan={columns.length}>
                <EmptyState title="No branches found" description="Try adjusting your search or filters." />
              </td>
            </tr>
          )}
          {!isLoading &&
            !isError &&
            branches.map((branch) => (
              <tr key={branch.id} className="border-b border-sap-border last:border-0 hover:bg-sap-bg/60">
                <td className="whitespace-nowrap px-4 py-3 text-sm font-medium text-sap-text">{branch.name}</td>
                <td className="whitespace-nowrap px-4 py-3 text-sm text-sap-text-muted">{branch.code}</td>
                <td className="whitespace-nowrap px-4 py-3 text-sm text-sap-text-muted">{branch.businessUnitName}</td>
                <td className="whitespace-nowrap px-4 py-3 text-sm text-sap-text-muted">{branch.manager}</td>
                <td className="whitespace-nowrap px-4 py-3 text-sm text-sap-text-muted">{branch.city}, {branch.country}</td>
                <td className="whitespace-nowrap px-4 py-3 text-sm">
                  <Badge tone={branch.status === 'Active' ? 'success' : 'danger'} dot>
                    {branch.status}
                  </Badge>
                </td>
                <td className="whitespace-nowrap px-4 py-3 text-sm">
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="sm" onClick={() => onEdit(branch)} aria-label="Edit branch">
                      <Pencil size={16} />
                    </Button>
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => onToggleStatus(branch)}
                      isLoading={togglingBranchId === branch.id}
                      aria-label={branch.status === 'Active' ? 'Deactivate branch' : 'Activate branch'}
                      className={branch.status === 'Active' ? 'text-sap-danger' : 'text-sap-success'}
                    >
                      {branch.status === 'Active' ? <PowerOff size={16} /> : <Power size={16} />}
                    </Button>
                  </div>
                </td>
              </tr>
            ))}
        </tbody>
      </table>
    </div>
  )
}
